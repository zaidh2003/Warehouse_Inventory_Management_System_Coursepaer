		...
		if( i == -1){
			...
			do {
				...
				while (!input.hasNextInt()) {
						System.out.println("Sell Quantity can only be positive number! Please type in proper positive quantity- ");
						input.next(); 
				}
	...
	public  void restockGrocery(String Name){
		...
		if( i == -1){
			...
			do {
				...
				while (!input.hasNextInt()) {
						System.out.println("Restock Quantity can only be positive number! Please type in proper positive quantity- ");
						input.next(); 
				}
				int amount = input.nextInt();
				input.nextLine();
